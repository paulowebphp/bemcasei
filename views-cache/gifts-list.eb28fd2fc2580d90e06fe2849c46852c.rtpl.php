<?php if(!class_exists('Rain\Tpl')){exit;}?><section id="gifts-list" class="site">


	<div class="container text-center">


		<div class="row centralizer">


			<div class="col-md-10 col-12">





				<div class="bottom2">

					<h1>Lista de Presentes</h1>

				</div>

				<div class="bottom2 blue">

					<h2>Sua Lista de Casamento convertida em dinheiro!</h2>

				</div>



				<div class="bottom2">
					<p>A Lista de Presentes do Bem Casei é super prática, além de ser uma das mais bonitas e elegantes:</p>
				</div>


				<div class="bottom3 centralizer">

					<ul> 

						<li>Personalize seus presentes</li> 

					    <li>Lista de Presentes Pronta com dezenas de itens cadastrados</li> 

					    <li>Acompanhe as transações por um painel de controle</li>

					    <li>Transferências gratuitas pra sua conta bancária</li>

					</ul>

				</div><!--gifts-section1-->



			</div><!--col-->

		</div><!--row-->









		<div class="row">


			<div class="col-md-12">



					<div class="table1 bottom2">

						<div class="bottom1">
							<strong>Simulação 1: Tarifa arcada pelo Casal:</strong>
						</div>


						<table>

							<thead>

								<tr> 

									<th><strong>Forma de pagamento</strong></th> 

									<th><strong>Valor do presente</strong></th> 

									<th><strong>Valor pago pelos convidados</strong></th> 

									<th><strong>Valor recebido pelos noivos</strong></th> 

								</tr>

							</thead> 

							<tbody> 

								<tr> 
								
									<td>Boleto Bancário e Cartão de Crédito à vista</td> 

									<td>R$ 100,00</td> 

									<td>R$ 100,00</td> 

									<td>R$ 95,01</td> 

								</tr>

							</tbody>

						</table>
					</div> 




					<div class="table1 bottom3">
						<div class="bottom1">
							<strong>Simulação 2: Tarifa arcada pelos Convidados:</strong> 
						</div>

						<table> 

							<thead> 

								<tr> 

									<th><strong>Forma de pagamento</strong></th>

									<th><strong>Valor do presente</strong></th> 

									<th><strong>Valor pago pelos convidados</strong></th> 

									<th><strong>Valor recebido pelos noivos</strong></th> 

								</tr> 

							</thead> 

							<tbody> 

								<tr> 

									<td>Boleto Bancário e Cartão de Crédito à vista</td> 

									<td>R$ 100,00</td> 

									<td>R$ 105,25</td> 

									<td>R$ 100,00</td> 


								</tr> 

							</tbody> 

						</table>
					</div>


			</div><!--col-->

		</div><!--row-->







		<div class="row bottom3">


			<div class="col-md-12">


				<div class="bottom2">
					
					<h3>Resgate dos Valores</h3>

				</div>


				<p>O valor  estará disponível para o casal transferir para sua conta bancária em 2 dias, no  caso pagamentos com Boleto Bancário, e 14 dias, no caso de pagamentos com  cartão de crédito à vista ou parcelado.</p>

				<p>Os saques  são gratuitos e podem ser realizados quantas vezes o casal desejar.</p>

			</div><!--col-->

		</div><!--row-->






		<div class="row bottom2">


			<div class="col-md-12">

				
				<h3>Vantagens da Lista de Presentes para o Casal</h3>


			</div><!--col-->

		</div><!--row-->






		<div class="row bottom3">



			<div class="col-md-6">


				<ul>

					<li>Uma das menores tarifas bancárias do mercado para compras com Boleto ou Cartão <span style="color:#6f007c"><strong>(4,29%)</strong></span></li>

					<li>Uma das menores taxas administrativas <span style="color:#6f007c"><strong>(0,70%)</strong></span></li>

					<li>Mesmo que o presente seja parcelado você recebe o valor integral</li>

				</ul> 


			</div><!--col-->





			<div class="col-md-6">

				<ul>

					<li>Um dos menores tempos de repasse para compras com cartão <span style="color:#6f007c"><strong>(14 dias)</strong></span> e boleto bancário <span style="color:#6f007c"><strong>(2 dias)</strong></span></li>

					<li>Transferências gratuitas para sua conta bancária</li>

				</ul>


			</div><!--col-->




		</div><!--row-->

  


		






		<div class="row centralizer bottom3">


			<div class="col-md-8 col-10">


				<div class="gifts-title3 purple">

					<p>Segurança e Credibilidade especialmente desenvolvidas pelo Bem Casei, para fazer do seu casamento e da sua lua de mel um sucesso, de forma simples e prática:</p>


				</div><!--gifts-section1-->


			</div><!--col-->


		</div><!--row-->






		<div class="row bottom2">


			<div class="col-md-12">

				
				<h3>Vantagens da Lista de Presentes para os Convidados</h3> 


			</div><!--col-->

		</div><!--row-->




		<div class="row bottom3">


			<div class="col-md-6"> 

				<ul class="gifts-list2">    

					<li>Várias bandeiras de cartão de crédito disponíveis</li>

					<li>Pagamento com Boleto Bancário</li>

					<li>Podem parcelar o presente em até 6 vezes</li>

					<li>Podem deixar uma mensagem de felicitação ao casal após a compra</li>

				</ul> 



			</div>






			<div class="col-md-6">

				<ul class="gifts-list2">

					<li>Compra em poucos cliques</li>

					<li>Ambiente com páginas criptografadas</li>

					<li>Segurança total, os dados de cartões são criptografados e não são armazenados</li>

					<li>Suporte total de nossa equipe para qualquer dúvida ou orientação</li>

				</ul>

			</div><!---->




		</div><!--row-->








		<div class="row">


			<div class="col-md-12">


				<div class="follow1 bottom2">
					
					<a href="/tarifas-condicoes"><h4>Ir Para Tarifas e Condições</h4></a>

				</div>

				
				<div class="follow2 bottom2">
					
					<a href="/criar-site"><h4>Criar Lista de Presentes</h4></a>

				</div>



			</div><!--col-->

		</div><!--row-->




	</div><!--container-->


</section>
