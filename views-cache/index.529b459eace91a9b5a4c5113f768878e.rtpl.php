<?php if(!class_exists('Rain\Tpl')){exit;}?><section id="banner">
    
    <div class="container">
        

        <div class="row centralizer">
            

            <div class="col-md-10">

                <div class="text-center">

                    <div class="bottom1">
                        <h3>VIVA O AMOR, SEJA ELE COMO FOR</h3>
                        
                    </div><!--inner-text-introduction-->

                    <div class="bottom2">
                        <h1>AMAR CASAR, O PRIMEIRO SITE DE CASAMENTO GAY DO BRASIL!</h1>
                    </div>

                    <a href="/criar-site"><button type="button" class="button1">Criar meu Site</button></a>

                </div><!--textIntroduction_-->

            </div><!--col-md-12-->

        </div>

    </div><!--container-->
    

</section>





  

   
   

<section id="features">

     <div class="container-fluid">

            
            <div class="row bottom3">

                <div class="col-md-12 text-center">
         
                    <h2>SITE DE CASAMENTO</h2>
          
                </div><!-- col-xs-12-->

            </div><!-- row-->

            


            <div class="row">



                <div class="col-md-3 text-center">

                    <div class="media">

                        <div class="text-center thumbnail-center"> 


                            <img src="/res/images/feature3.jpg" class="media-object">

                            <div class="media-body">

                                <h3 class="media-heading feature-box">LISTA DE PRESENTES EM DINHEIRO</h3>
                       
                                <p>Crie uma lista de presentes, envie para seus convidados e converta os valores recebidos em dinheiro</p>

                            </div>

                        </div>
                        

                    </div><!--media-->

                </div><!--col-lg-3-->




                <div class="col-md-3 text-center">

                    <div class="media">

                        <div class="text-center thumbnail-center"> 


                            <img src="/res/images/feature2.jpg" class="media-object">

                            <div class="media-body">

                                <h3 class="media-heading feature-box">LINDOS TEMPLATES</h3>
                       
                                <p>Encante seus convidados com um site de casamento elegante e atraente</p>

                            </div>

                        </div>
                        

                    </div><!--media-->

                </div><!--col-lg-3-->

                


                <div class="col-md-3 text-center">

                    <div class="media">

                        <div class="text-center thumbnail-center"> 


                            <img src="/res/images/feature2.jpg" class="media-object">

                            <div class="media-body">

                                <h3 class="media-heading feature-box">CONFIRMAÇÃO DE PRESENÇA</h3>
                       
                                <p>Organize e receba de forma prática e confortável as confirmações de presença</p>


                            </div>

                        </div>
                        

                    </div><!--media-->

                </div><!--col-lg-3-->




                
        

              




                <div class="col-md-3 text-center">

                    <div class="media">

                        <div class="text-center thumbnail-center"> 


                            <img src="/res/images/feature4.jpg" class="media-object">

                            <div class="media-body">

                                <h3 class="media-heading feature-box">MURAL DE RECADOS</h3>
                       
                                <p>Receba mensagens de felicitações de seus familiares e amigos num mural</p>

                            </div>

                        </div>

                        
                    </div><!--media-->

                </div><!--col-lg-3-->
      
            </div><!--row-->
    
    </div><!--container-->

</section>






<section id="create-list">

    <div class="container">

        <div class="row">

            <div class="col-md-6 text-center">

                <p>
                    Crie sua Lista e receba seus presentes em dinheiro de maneira prática e fácil, com a moderna Lista de Presentes do Amar Casar.<br>
                    Comodidade para o casal, segurança e confiabilidade para os convidados
                </p>

            </div><!--col-->


            <div class="col-md-6 text-center"> 


                <div class="row bottom2">
                    
                    <div class="col-md-12">
                        
                        <a href="/lista-presentes"><button type="button" class="button1">Lista de Presentes</button></a>

                    </div>

                </div>

                <div class="row bottom2">
                    
                    <div class="col-md-12">
                        
                        <a href="/criar-site"><button type="button" class="button1">Criar Lista</button></a>

                    </div>

                </div>

            </div><!--col-->

        </div><!--row-->

    </div><!--container-->

</section>










<section id="advantages">

    <div class="container">

        <div class="row bottom3">

            <div class="col-md-12 text-center">

                <h2>FUNCIONALIDADES</h2>

            </div><!--col-lg-12-->

        </div><!--row-->

        <div class="row">

            <div class="col-lg-8 col-12">

                <h3>TEMPLATES EXCLUSIVOS PARA</h3>

                <h3>QUEM QUER CASAR E AMAR</h3>

                <p>&nbsp;</p>

                <p>Seu site de casamento merece um design exclusivo Amar Casar. </p>

                <p>Escolha seu template e personalize do seu jeito! </p>

                <p>&nbsp;</p>

                <a href="/modelos-templates"><button type="button" class="btn btn-md btn-advantage btn-advantage1">
                    VER TEMPLATES
                </button></a>

                <a href="/site-casamento"><button type="button" class="btn btn-md btn-advantage btn-advantage2">
                    VER FUNCIONALIDADES
                </button></a>

                <h2>&nbsp;</h2>

            </div><!--col-lg-8-->

            <div class="col-lg-4 col-12">

                <div id="box-price">

                    <div id="box-price-logo">
                        <img src="/res/images/logo/logo-white.png">             
                    </div>


                    <ul id="advantages-features">

                        <li id="box-header">Tenha acesso a um sistema on-line que irá lhe auxiliar a divulgar seu casamento</li>

                        <li><img src="/res/images/red-arrow.png">&nbsp;&nbsp;Receba Presentes em Dinheiro</li>

                        <li><img src="/res/images/red-arrow.png">&nbsp;&nbsp;Confirmação de Presença on-line</li>      

                        <li><img src="/res/images/red-arrow.png">&nbsp;&nbsp;Templates Modernos</li>

                        <li><img src="/res/images/red-arrow.png">&nbsp;&nbsp;Saques Gratuitos</li>

                        <li>&nbsp;&nbsp;<b>E muito mais!</b></li>

                    </ul>


                    <div id="box-footer">              

                        <a href="/planos"><strong>COMECE JÁ</strong></a>

                    </div><!--active-->

                </div><!--price_block-->

            </div><!--col-md-4--> 

        </div><!--row-->

</section>








<section id="confirmation">

    <div class="container text-center">

        



        <div class="row">

            <div class="col-md-6 col-12 text-center centralizer">
                
                <h2>Olá, você é Convidado?</h2>

            </div>

            <div class="col-md-6 col-12 text-center"> 
            

                <div class="row bottom2">
            
                    <div class="col-12">
                        
                        <a href="/buscar"><button type="button" class="button1">

                            Presenteie o Casal

                        </button></a>
                    </div>
                </div>


                <div class="row bottom2">
            
                    <div class="col-12">
                        
                        <h3>ou</h3>
                    </div>
                </div>


                <div class="row bottom2">
            
                    <div class="col-12">
                        
                        <a href="/buscar"><button type="button" class="button1">      

                            Confirme sua Presença 

                        </button></a>
                    </div>
                </div>


            </div><!--col-md-6-->

        </div><!--row-->

    </div><!--container-->
<!-- # End / Contet Text #  -->
</section>





